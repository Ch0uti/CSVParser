...
public class CSVParser {
    ...
    public func wirite(toFilePath path: String) throws {
        try rows.map { $0.joined(separator: String(self.delimiter)) }.joined(separator: String(lineSeparator)).write(to: URL(fileURLWithPath: path), atomically: false, encoding: .utf8)
    }
     ...
     */
    public static func jsonToCSVString(jsonData: Data) throws -> String {
        guard let jsonObj = try JSONSerialization.jsonObject(with: jsonData, options: .allowFragments) as? [[String: Any]] else {
...
extension CSVParser: Sequence {
    public func makeIterator() -> CSVParserIterator {
        return CSVParserIterator(rows: rows)
    }
}
...
extension CSVParser: RangeReplaceableCollection {
    public func replaceSubrange<C>(_ subrange: Range<Int>, with newElements: C) where C: Collection, C.Iterator.Element == [String] {
        _rows.replaceSubrange(subrange, with: newElements)
    }

    public func reserveCapacity(_ n: Int) {
        _rows.reserveCapacity(n)
    }
}
